...
const getTimeSlotCategory = (shift: Shift): string => {
...
const DailyRoster: React.FC<DailyRosterProps> = ({ shifts, staff, selectedDate, onShiftClick }) => {
  const getStaffMember = (staffId: string | undefined) => {
    if (!staffId) return null;
    return staff.find(s => s.id === staffId) || null;
  };

  const shiftsBySlot: { [slot: string]: Shift[] } = {};
